...
def generate_unique_mutations(
    ...
    for sequence in tqdm(reference_sequences):
        ...
        while len(unique_mutations) < num_mutations and attempts < max_attempts:
            ...
            num_positions = np.random.randint(
                1, max(2, int(len(sequence) * max_mutation_distance)) + 1
            )
            ...
            for _ in range(num_positions):
                ...
                valid_choices = [
                    aa for aa in all_amino_acids if aa != mutation[mutation_position]
                ]
                if valid_choices:
            ...
            if (
                distance_func(mutation_str, sequence) / len(sequence)
                <= max_mutation_distance
                and mutation_str not in unique_mutations
                and mutation_str != sequence
            ):
        ...
        if len(mutations_list) < num_mutations:
            print(
                f"Warning: Could only generate {len(mutations_list)} unique mutations for sequence {sequence}"
            )
...
def main():
    ...
    unique_mutations = generate_unique_mutations(
        reference_sequences, num_mutations=1_000
    )
    all_mutations_flatten = [x for xs in unique_mutations for x in xs]

    pd.DataFrame(all_mutations_flatten).to_csv(
        "generated_inits/test_random_seed900_999_1k.csv", index=False, header=False
    )
